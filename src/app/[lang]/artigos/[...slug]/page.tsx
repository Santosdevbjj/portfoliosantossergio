...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    description: `Artigo técnico sobre engenharia de software no idioma ${lang}.`,
    openGraph: {
 ...
 * Garante que sempre haja um retorno, mesmo que a API falhe no build.
 */
export async function generateStaticParams() {
  try {
    ...
    if (!articles || articles.length === 0) {
      // Retorno mínimo para evitar quebra do build do Next 16
    ...
    return SUPPORTED_LOCALES.map((locale) => ({ lang: locale, slug: ["error"] }));
  }
}

export default async function ArtigoDetalhePage({ params }: PageProps) {
  // Aguarda params e consome headers para estabilidade de renderização no Node 24
  ...
  // Sanitização da URL para o GitHub Raw
  const fileName = pathStr.endsWith('.md') ? pathStr : `${pathStr}.md`;
        ...
        <article className="max-w-4xl mx-auto">
          ...
          <footer className="mt-32 pt-16 border-t border-zinc-200 dark:border-zinc-900">
            ...
            <div className="mt-24 flex flex-col items-center opacity-30 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-500">
              <img src="/images/trofeus-vencedor-dio.png" alt="Award" className="h-16 mb-4" />
