...
export const CareerHighlights = ({ dict }: CareerHighlightsProps) => {
  const { about } = dict;
  const items = about.sections.highlights.items;

  // Mapeamento de ícones para cada destaque para dar um ar técnico
  const icons = [
    <Clock key="0" className="text-amber-500" size={24} />,
    <Server key="1" className="text-blue-500" size={24} />,
    <ShieldCheck key="2" className="text-emerald-500" size={24} />
  ];
  ...
  return (
    <div className="mt-16 space-y-8">
      ...
      {/* Estatística Rápida de Impacto (Data Analytics Style) */}
      <div className="p-8 rounded-[2.5rem] bg-blue-600 text-white flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-blue-600/20">
        <div className="flex items-center gap-6">
          ...
          <div>
            <p className="text-blue-100 text-sm font-bold uppercase tracking-wider mb-1">Impacto Acumulado</p>
            <p className="text-3xl font-black">+15 Anos em Ambientes Críticos</p>
          </div>
        </div>
        <div className="h-px w-full md:h-12 md:w-px bg-white/20" />
        <div className="text-center md:text-left">
          <p className="text-4xl font-black mb-1">99.5%</p>
          <p className="text-blue-100 text-xs font-bold uppercase tracking-widest text-nowrap">Disponibilidade Operacional</p>
        </div>
        <div className="h-px w-full md:h-12 md:w-px bg-white/20" />
        <div className="text-center md:text-left">
          <p className="text-4xl font-black mb-1">2.9K</p>
          <p className="text-blue-100 text-xs font-bold uppercase tracking-widest text-nowrap">Horas/Ano Automatizadas</p>
        </div>
      </div>
    </div>
  );
