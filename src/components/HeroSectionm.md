 ...
 * Versão Universal: Resolve o conflito entre ProxyClient.tsx e page.tsx
 * Compatível com Next.js 16 e TS 7.0
 */

import { Dictionary, Locale } from "@/types/dictionary";
import { getNavHash, NavSection } from "@/domain/navigation";

// Interface Híbrida: Aceita tanto o objeto 'dict' completo quanto as props separadas
interface HeroSectionProps {
  // Forma do ProxyClient.tsx
  dict?: Dictionary; 
  
  // Forma do page.tsx (conforme erro no log)
...
export default function HeroSection({ 
   ...
   * Se vier 'dict' (do ProxyClient), usamos ele. 
   * Se vier 'content' (do page.tsx), usamos as props diretas.
   */
  const heroData = content || dict?.hero;
  const metricsData = metrics || dict?.metrics;
  const aboutData = about || dict?.about;
  const contactData = dict?.contact; // Fallback se necessário

  // Prevenção de erro caso os dados não existam durante o build
    ...
    >
      <div className="container relative z-10 mx-auto flex flex-col items-center text-center">
        ...
        <div className="flex flex-col w-full gap-4 sm:flex-row sm:w-auto sm:gap-6">
          ...
          >
            {/* Usa o label do dicionário ou fallback fixo por idioma */}
            {contactData?.cvLabel || (lang === 'pt-BR' ? 'Baixar CV' : 'Download CV')}
          </a>
        </div>

        {/* MÉTRICAS */}
        {metricsData?.availability && aboutData?.stats && (
          <div className="mt-16 flex items-center gap-2 text-sm font-medium text-slate-500 dark:text-slate-500">
            ...
            <span>
              {aboutData.stats.availabilityLabel}: <b className="text-slate-700 dark:text-slate-300">{metricsData.availability}</b>
            </span>
          </div>
        )}
      </div>
