...
// Importamos apenas ícones que EXISTEM na v1.7.0 (Removidos: Github, Linkedin)
import { Mail, Copy, Check, FileText, ExternalLink } from 'lucide-react';
...
export default function ContactSection({ contact, common, locale }: ContactSectionProps) {
  ...
  const copiedLabel = useMemo(() => {
    const labels: Record<string, string> = {
      'pt-BR': 'Copiado!', 'en-US': 'Copied!', 'es-ES': '¡Copiado!', 'es-AR': '¡Copiado!', 'es-MX': '¡Copiado!'
    };
          ...
          <div className="relative z-10 flex flex-col items-center gap-12 lg:flex-row lg:justify-between">
            <div className="flex-1 text-center lg:text-left">
              ...
              <div className="flex flex-col flex-wrap items-center justify-center gap-4 sm:flex-row lg:justify-start">
                <a href={`mailto:${email}?subject=Contact from ${origin}`} className="flex w-full items-center justify-center gap-3 rounded-2xl bg-white px-8 py-4 text-lg font-black text-blue-600 shadow-lg transition-all hover:scale-105 active:scale-95 sm:w-auto">
                  <Mail className="size-6" /> {contact.cta}
                </a>

                <button onClick={copyToClipboard} className={`flex w-full items-center justify-center gap-3 rounded-2xl border px-6 py-4 text-base font-bold transition-all active:scale-95 sm:w-auto cursor-pointer ${copied ? 'bg-emerald-500 border-emerald-400 text-white' : 'bg-blue-800/40 border-white/20 text-white hover:bg-blue-800/60'}`}>
                  {copied ? <Check className="size-5" /> : <Copy className="size-5 opacity-70" />}
                  {copied ? copiedLabel : contact.emailLabel}
                </button>

                <Link href={`/${locale}/resume`} className="flex w-full items-center justify-center gap-3 rounded-2xl border border-white/30 bg-white/10 px-6 py-4 text-base font-bold text-white backdrop-blur-md transition-all hover:bg-white/20 sm:w-auto">
                  <FileText className="size-5" /> {contact.cvLabel}
                </Link>
              </div>
            </div>

            {/* ICONS DE MARCA: Usando SVGs Oficiais (Solução definitiva para 2026) */}
            <div className="flex flex-row gap-4 lg:flex-col lg:gap-6">
              <a href={linkedinUrl} target="_blank" rel="noopener noreferrer" aria-label="LinkedIn" className="group rounded-2xl bg-white p-5 text-blue-600 shadow-xl hover:-translate-y-2 transition-all duration-300">
              ...
              <a href={githubUrl} target="_blank" rel="noopener noreferrer" aria-label="GitHub" className="group rounded-2xl bg-slate-900 p-5 text-white shadow-xl hover:-translate-y-2 transition-all duration-300">
                <svg className="size-8 md:size-10 fill-current" viewBox="0 0 24 24">
