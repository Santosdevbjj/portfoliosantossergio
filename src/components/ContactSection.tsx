 ...
 * Design premium com foco em ergonomia mobile e acessibilidade global.
 */
export const ContactSection = ({ lang, dict }: ContactSectionProps) => {
  ...
  // Mapeamento dinâmico via dicionário centralizado
  const content = dict?.contact || {};
  
  // Rótulos de utilitários vindo do dicionário (ou fallback seguro)
  const labels = {
    copy: dict?.common?.copy || { pt: 'Copiar', en: 'Copy', es: 'Copiar' }[lang],
    copied: dict?.common?.copied || { pt: 'Copiado!', en: 'Copied!', es: '¡Copiado!' }[lang]
  };
  ...
  return (
    <section id="contact" className="py-20 lg:py-32 bg-slate-50 dark:bg-[#020617] transition-colors duration-500 overflow-hidden relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* CONTAINER PRINCIPAL: CTA de Alto Impacto */}
        <div className="bg-blue-600 dark:bg-blue-700 rounded-[2.5rem] lg:rounded-[4rem] p-8 sm:p-12 md:p-16 lg:p-24 shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-16 overflow-hidden relative border border-white/10">
          ...
          {/* COLUNA ESQUERDA: Texto e Ações Primárias */}
          <div className="max-w-xl text-center lg:text-left relative z-20">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              ...
              {/* Botão de Cópia: Design Adaptativo */}
              <button 
                onClick={copyToClipboard}
                className="group flex items-center justify-center gap-3 bg-blue-800/40 text-white border border-white/20 px-6 py-4 md:px-8 md:py-5 rounded-2xl font-bold hover:bg-blue-800/60 transition-all backdrop-blur-md active:scale-95 min-w-[140px]"
              >
                {copied ? (
                  ...
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <span className="text-[11px] md:text-sm font-mono tracking-tight truncate max-w-[180px]">
                      {email}
                    </span>
                  </div>
                )}
          ...
          {/* COLUNA DIREITA: Social Networking Box */}
        ...
        {/* STATUS: Badge de Disponibilidade em tempo real */}
