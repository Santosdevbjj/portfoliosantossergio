...
export const LanguageSwitcher = () => {
  ...
  // Função segura para calcular a nova rota ao trocar de idioma
  const getNewPath = (lang: string) => {
    if (!pathname) return `/${lang}`
    const segments = pathname.split('/')
    // Garante que o primeiro segmento após a barra seja o idioma
    segments[1] = lang
        ...
        <Link
          ...
          className={`px-3 py-1 text-xs font-bold rounded-full transition-all duration-200 ${
            currentLang === lang.code
              ? 'bg-blue-600 text-white shadow-sm' // Ajustado para blue-600 para garantir visibilidade
